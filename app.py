...
def page_not_found(e):
    return (
        "Страница не найдена. Подумайте, нужна ли она вам. Если да - попробуйте ещё раз, но честно говоря, "
        "вряд ли что-то изменится. TEST",
        404,
    )
...
def page_feed():
    ...
    return render_template(
        "index.html", posts=posts, bookmarks_num=len(read_json(BOOKMARKS_PATH))
    )
    ...
    if request.method == "GET":
        for post in tags_to_links(read_json(POSTS_PATH)):
            if postid == post["pk"]:
                ...
                return render_template(
                    "post.html", **post, comments=comments, comments_num=len(comments)
                )
        abort(404)
...
def page_search():
    ...
    results = tags_to_links(
        get_posts_by_search(posts_with_comments_count(POSTS_PATH, COMMENTS_PATH), s)
    )
    if len(results) > 10:
    ...
    return render_template(
        "search.html", results=limited_results, results_num=len(results)
    )


@app.route("/users/<username>/")
def page_user(username):
    user_posts = tags_to_links(
        get_posts_by_username(
            posts_with_comments_count(POSTS_PATH, COMMENTS_PATH), username
        )
    )
...
def page_tag(tagname):
    tag_posts = tags_to_links(
        get_posts_by_tag(posts_with_comments_count(POSTS_PATH, COMMENTS_PATH), tagname)
    )
...
@app.route("/bookmarks")
def page_bookmarks():
    bookmarks = tags_to_links(
        get_posts_by_bookmarks(
            posts_with_comments_count(POSTS_PATH, COMMENTS_PATH), BOOKMARKS_PATH
        )
    )
